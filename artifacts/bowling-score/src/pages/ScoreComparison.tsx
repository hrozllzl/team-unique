...
function calcAvg(
  ...
  const filtered = records.filter(
    (r) => r.memberId === memberId && r.date >= from && r.date <= to
  );
...
export default function ScoreComparison() {
  ...
  const defaultPeriods = useMemo(() => {
    ...
    const mid = Math.max(1, Math.floor(gameDates.length / 2));
    return {
      baseFrom: gameDates[0],
      baseTo: gameDates[mid - 1],
      cmpFrom: gameDates[Math.min(mid, gameDates.length - 1)],
      cmpTo: gameDates[gameDates.length - 1],
    };
  }, [gameDates]);

  const [baseFromYear, setBaseFromYear] = useState(() => defaultPeriods?.baseFrom.slice(0, 4) ?? "");
  const [baseFromDate, setBaseFromDate] = useState(() => defaultPeriods?.baseFrom ?? "");
  const [baseToYear, setBaseToYear] = useState(() => defaultPeriods?.baseTo.slice(0, 4) ?? "");
  const [baseToDate, setBaseToDate] = useState(() => defaultPeriods?.baseTo ?? "");

  const [cmpFromYear, setCmpFromYear] = useState(() => defaultPeriods?.cmpFrom.slice(0, 4) ?? "");
  const [cmpFromDate, setCmpFromDate] = useState(() => defaultPeriods?.cmpFrom ?? "");
  const [cmpToYear, setCmpToYear] = useState(() => defaultPeriods?.cmpTo.slice(0, 4) ?? "");
  const [cmpToDate, setCmpToDate] = useState(() => defaultPeriods?.cmpTo ?? "");
  ...
  const stats = useMemo((): MemberStat[] => {
    ...
    return members.map((member) => {
      const { avg: baseAvg, count: baseCount } = calcAvg(records, member.id, baseFromDate, baseToDate);
      const { avg: cmpAvg, count: cmpCount } = calcAvg(records, member.id, cmpFromDate, cmpToDate);
  ...
  }, [members, records, baseFromDate, baseToDate, cmpFromDate, cmpToDate]);

  const sortedStats = useMemo(() => {
    return [...stats].sort((a, b) => {
      if (sortField === "name") {
        const cmp = a.member.name.localeCompare(b.member.name, "ko");
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-foreground">점수 비교</h1>
        </div>
